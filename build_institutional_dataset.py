...
symbols = [
    ...
    'DOGEUSDT', 'AVAXUSDT', 'SHIBUSDT', 'LINKUSDT', 'MATICUSDT', 'UNIUSDT'
]
processed_dfs = []
...
def fetch_binance_futures_data(symbol, interval="1h", days=30):
    ...
    params = {"symbol": symbol, "interval": interval, "startTime": start_time, "endTime": end_time, "limit": 1500}
    res = session.get(ohlcv_url, params=params, timeout=10)
    res.raise_for_status()
    data = res.json()
    
    df_ohlcv = pd.DataFrame(data, columns=[
    ...
    params = {"symbol": symbol, "period": interval, "startTime": start_time, "endTime": end_time, "limit": 500}
...
def apply_frac_diff_optimized(series, d, thres=1e-4):
    ...
    width = len(w)
    if len(series) < width: return pd.Series(np.nan, index=series.index)

...
def load_and_prepare_data(csv_path, sequence_length=60):
    ...
    features = ['close_fd', 'rsi_14', 'MACD_12_26_9', 'atr_14']
    target = 'tbm_label'
...
class ResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels, dilation):
        ...
        self.causal_conv1 = CausalConv1d(in_channels, out_channels, kernel_size=3, dilation=dilation)
        self.relu1 = nn.ReLU()
        self.causal_conv2 = CausalConv1d(out_channels, out_channels, kernel_size=3, dilation=dilation)
        self.relu2 = nn.ReLU()
...
# 4. Training Loop
def train_model(model, train_loader, val_loader, pos_weight, num_epochs=100, patience=10):
    ...
    for epoch in range(num_epochs):
        ...
        for X_batch, y_batch in train_loader:
            ...
            sample_weights = torch.where(y_batch == 1.0, torch.tensor(pos_weight, device=device), torch.tensor(1.0, device=device))
...
if __name__ == "__main__":
    ...
    # If file doesn't exist, create dummy data for execution safety
    if not os.path.exists(csv_file):
        print(f"WARNING: '{csv_file}' not found. Generating dummy dataset for execution test...")
        np.random.seed(42)
        dummy_data = {
            'close_fd': np.random.randn(2000),
            'rsi_14': np.random.randn(2000) * 10 + 50,
            'MACD_12_26_9': np.random.randn(2000),
            'atr_14': np.random.randn(2000) * 5 + 10,
            'tbm_label': np.random.choice([0, 1], size=2000, p=[0.62, 0.38])
        }
        pd.DataFrame(dummy_data).to_csv(csv_file, index=False)
        
    seq_len = 60
    train_loader, val_loader, test_loader, pos_weight = load_and_prepare_data(csv_file, sequence_length=seq_len)
    ...
    model = TCN(input_size=input_size, num_channels=num_channels, sequence_length=seq_len)
    
    train_model(model, train_loader, val_loader, pos_weight, num_epochs=100, patience=10)

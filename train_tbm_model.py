...
# Configuration
DATA_FILE = "labeled_alpha_dataset.csv"
TARGET_COL = "tbm_label"
# DROP_COLS — columns that must NEVER be features:
#   • target_return_4h    : the leak — equals close.shift(-4)−close
#   • barrier_hit_time    : created by TBM, knowing time-to-barrier leaks future
#   • close_fd            : legacy column name, may collide with close_fd_04
DROP_COLS = [
    "timestamp",
    "target_return_4h",
    "barrier_hit_time",
    "close_fd",
]  # tbm_label is handled separately
MODEL_SAVE_PATH = "tbm_xgboost_model.json"


def load_and_prepare_data():
    """
    Loads the labeled dataset and prepares it for time-series training.
    """
...
def main():
    try:
        ...
        if count_1 == 0:
            raise ValueError(
                "Training set has no positive (1) samples. Cannot train model."
            )
        ...
        print(
            f"Error: Could not find '{DATA_FILE}'. Please ensure you run tbm_labeler.py first."
        )

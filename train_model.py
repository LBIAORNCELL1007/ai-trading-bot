...
# DROP_COLS — features that MUST never be fed to the classifier:
#   • timestamp           : an index, not a feature
#   • close_fd            : redundant with close_fd_04 (legacy column name)
#   • barrier_hit_time    : created during TBM labelling — knowing how many
#                           bars the barrier-hit took is a direct future leak
#   • target_return_4h    : a regression label = close.shift(-4) − close.
#                           If left in features, the classifier sees the
#                           future return value and trivially "predicts"
#                           with 100% accuracy on train (collapses on test).
#                           Confirmed leak: built in build_dataset.py:229.
#   • barrier_hit_label   : the integer barrier-hit label, sometimes left in
#                           via different builders.
DROP_COLS = [
    "timestamp",
    "close_fd",
    "barrier_hit_time",
...
def main():
    ...
    study = optuna.create_study(
        direction="maximize", study_name="XGBoost_Binary_Classification"
    )
